...
export const ClientsView: React.FC<ClientsViewProps> = ({
  ...
  const [tagColors, setTagColors] = useState<Record<string, string>>({
    ...
    Accident: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20",
    "Critical Illness":
    ...
    Maid: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20",
    Sunlife: "bg-blue-500/10 text-blue-400 border-blue-500/20",
    Prudential: "bg-blue-500/10 text-blue-400 border-blue-500/20",
    AIA: "bg-blue-500/10 text-blue-400 border-blue-500/20",
  });
  ...
  const [newSystemTagColor, setNewSystemTagColor] = useState(
    TAG_COLORS[5].class,
  ); // Default to Slate
  ...
  const [activeTagInputClient, setActiveTagInputClient] = useState<
    string | null
  >(null);
  ...
  const [addingPolicyToClient, setAddingPolicyToClient] =
  ...
  const filteredClients = clients.filter((client) => {
    ...
    if (searchTerm) {
      ...
      const matchesTag = client.tags.some((t) =>
  ...
  const handleAddSystemTag = () => {
    if (
      newSystemTagValue.trim() &&
      !systemTags.includes(newSystemTagValue.trim())
    ) {
    ...
    if (newTag !== oldTag) {
      clients.forEach((client) => {
        if (client.tags.includes(oldTag)) {
          const updatedTags = client.tags.map((t) =>
            t === oldTag ? newTag : t,
          );
    ...
    if (newClientForm.name) {
      if (isEditingClient && editClientId) {
        ...
        const newClient: Client = {
          ...
          birthday:
            newClientForm.birthday || new Date().toISOString().split("T")[0],
          totalPolicies: 0,
  ...
  const handleCreatePolicy = () => {
    if (
      ...
      newPolicyForm.planName &&
      newPolicyForm.policyNumber
    ) {
      const policy: PolicyData = {
        ...
        policyAnniversaryDate:
          newPolicyForm.policyAnniversaryDate ||
  ...
  const handleAddRiderToForm = () => {
    setNewPolicyForm((prev) => ({
      ...
      riders: [
        ...(prev.riders || []),
        { name: "", type: prev.type || "Medical", premiumAmount: 0 },
      ],
    }));
  };

  const handleUpdateRiderInForm = (
    index: number,
    field: keyof Rider,
    value: any,
  ) => {
              ...
              <div>
                ...
                <div className="relative mb-2">
                  ...
                  >
                    ...
                    {systemTags
                      .filter((t) => !newClientForm.tags?.includes(t))
                      .map((tag) => (
                        <option key={tag} value={tag}>
                          {tag}
                        </option>
