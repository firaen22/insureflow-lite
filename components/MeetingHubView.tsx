        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Timeline of all client interactions and reviews</p>
                </div>
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-500 shadow-sm"
                />
            </div>
            ...
            <div className="space-y-4">
                {filteredLogs.length > 0 ? filteredLogs.map((log, idx) => (
                    <div
                        key={log.id}
                        className="bg-white rounded-xl border border-slate-200 p-5 hover:border-brand-300 transition-all hover:shadow-md group relative overflow-hidden"
                    >
                        <div className="absolute left-0 top-0 bottom-0 w-1 bg-slate-100 group-hover:bg-brand-500 transition-colors" />
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                            <div className="space-y-2 flex-1">
                                ...
                                <h3
                                    ...
                                    className="font-bold text-slate-800 flex items-center gap-1.5 cursor-pointer hover:text-brand-600 transition-colors"
                                >
                                ...
                                {log.rawNotes && (
                                    <details className="mt-2 group/notes">
                                        <summary className="text-[11px] text-slate-400 cursor-pointer hover:text-slate-600 font-bold uppercase tracking-tight list-none flex items-center gap-1">
                                            <MessageSquare className="w-3 h-3" /> View Rough Notes
                                        </summary>
                                        <div className="mt-2 p-3 bg-slate-50 rounded-lg text-xs text-slate-500 whitespace-pre-wrap leading-normal font-mono">
                                            {log.rawNotes}
                                        </div>
                                    </details>
                                )}
                    ...
                    <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-slate-300">
                        ...
                        <p className="text-slate-500 font-medium">{t.noLogs}</p>
                    </div>
                )}
            </div>
        </div>
