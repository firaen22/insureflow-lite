        ...
        <div className="space-y-6">
                    ...
                    {[
                        ...
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all ${filterDays === btn.value
                                ? 'bg-slate-900 text-slate-900 dark:text-white shadow-sm dark:shadow-xl shadow-black/40'
                                : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-white/[0.02]'
                                }`}
                        >
                            {btn.label}
                        </button>
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full pl-10 pr-4 py-2 bg-white/80 dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-500 shadow-lg shadow-black/20"
                />
            </div>
            ...
            <div className="space-y-4">
                {filteredLogs.length > 0 ? filteredLogs.map((log, idx) => (
                    <div
                        key={log.id}
                        className="bg-white/80 dark:bg-white/5 backdrop-blur-xl rounded-xl border border-slate-200 dark:border-white/10 p-5 hover:border-brand-300 transition-all hover:shadow-sm dark:shadow-xl shadow-black/40 group relative overflow-hidden"
                    >
                        <div className="absolute left-0 top-0 bottom-0 w-1 bg-white dark:bg-white/[0.05] group-hover:bg-slate-100 dark:bg-white/100 transition-colors" />
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                            <div className="space-y-2 flex-1">
                                    ...
                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${log.type === 'Policy Review' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' :
                                        log.type === 'Intro' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                            log.type === 'Claim' ? 'bg-red-50 text-red-700 border-red-100' :
                                                'bg-slate-50 dark:bg-white/[0.02] text-slate-700 dark:text-slate-200 border-slate-200 dark:border-white/10'
                                        }`}>
                                ...
                                <h3
                                    ...
                                    className="font-bold text-slate-900 dark:text-white flex items-center gap-1.5 cursor-pointer hover:text-slate-900 dark:text-white transition-colors"
                                >
                                ...
                                <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed italic border-l-2 border-slate-100 dark:border-white/5 pl-3">
                                    "{log.summary}"
                                </p>
                    ...
                    <div className="text-center py-20 bg-white/80 dark:bg-white/5 backdrop-blur-xl rounded-2xl border border-dashed border-slate-300 dark:border-white/20">
                        <Clock className="w-12 h-12 text-slate-700 dark:text-slate-200 mx-auto mb-4" />
                        <p className="text-slate-500 dark:text-slate-400 font-medium">{t.noLogs}</p>
                    </div>
                )}
            </div>
        </div>
