    ...
    try {
        ...
        const policyRows = policies.map(p => [
            ...
            JSON.stringify({
                ...
                accidentPhysioLimitType1: p.accidentPhysioLimitType1,
                accidentPhysioLimitAmount1: p.accidentPhysioLimitAmount1,
                accidentPhysioLimitType2: p.accidentPhysioLimitType2,
                accidentPhysioLimitAmount2: p.accidentPhysioLimitAmount2,
                accidentBonesettingLimit: p.accidentBonesettingLimit,
                accidentAcupunctureLimit: p.accidentAcupunctureLimit,
...
export const shareClientData = async (client: Client, policies: PolicyData[], shareEmail?: string) => {
    try {
        ...
        // 4. Add Permission (Share with Email)
        if (shareEmail && shareEmail.includes('@')) {
            try {
                await window.gapi.client.drive.permissions.create({
                    ...
                    fields: 'id',
                    resource: {
                        role: 'reader', // 'writer' for edit access, 'reader' for view only
                        type: 'user',
                        emailAddress: shareEmail
                    },
                    emailMessage: `Here is the joint case file for client: ${client.name}. Generated by InsureFlow Lite.`,
                    sendNotificationEmail: true
                });
            } catch (permError) {
                console.warn("Failed to add permission automatically (Scope might be limited):", permError);
                // Non-fatal, user can still share manually via URL
            }
        }
